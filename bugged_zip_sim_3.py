...
if __name__ == "__main__":
    ...
    if not headless:
        ...
        camera = Camera((CAMERA_AHEAD_M, 0.0))
    ...
    for _ in range(num_trees):
        while True:
            # Round the position to the nearest tenth of a meter. This keeps the sprites from jumping around while
            # drawing due to floating point round-off to the nearest pixel.
            tree_pos = (round(random.uniform(*TREE_X_BOUNDS) % WORLD_LENGTH, 1),
                        round(random.uniform(0, WORLD_WIDTH) % WORLD_LENGTH, 1))
